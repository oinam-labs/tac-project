    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between no-print">
                <div className="flex items-center gap-2">
                    <div className="flex items-baseline">
                        <span className="text-2xl font-black text-foreground tracking-tighter">TAC</span>
                ...
                <div className="flex items-center gap-3">
                    {!isGenerated ? (
                        <Button onClick={handleGenerate} className="bg-primary hover:bg-primary text-white font-black uppercase tracking-wide">
                            <Save className="w-4 h-4 mr-2" /> Generate Invoice
                        </Button>
                    ) : (
                        <Button onClick={handlePrint} variant="outline" className="border-foreground text-foreground font-black uppercase tracking-wide hover:bg-foreground hover:text-white transition-colors">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full no-print">
                                ...
                                <CardContent className="p-6">
                                    <div className="grid grid-cols-3 gap-6 mb-6">
                                        <div className="space-y-1.5">
                                            <Label className="text-[10px] uppercase font-bold text-muted-foreground">Nature of Goods</Label>
                                            <Input
                                                value={data.natureOfQuantity}
                                                onChange={(e) => setData(p => ({ ...p, natureOfQuantity: e.target.value }))}
                                                className="font-bold border-muted"
                                            />
                                        </div>
                                        <div className="space-y-1.5">
                                            <Label className="text-[10px] uppercase font-bold text-muted-foreground">Declared Value</Label>
                                            <Input
                                        ...
                                        <div className="space-y-1.5">
                                            <Label className="text-[10px] uppercase font-bold text-muted-foreground">Booking Remarks</Label>
                                            <Input
                ...
                <TabsContent value="invoice" className="flex justify-center bg-muted/50 p-8 rounded-xl border border-dashed border-muted min-h-[500px]">
                    <InvoicePrint data={data} totals={totals} />
                </TabsContent>

                <TabsContent value="label" className="flex justify-center bg-muted/50 p-8 rounded-xl border border-dashed border-muted min-h-[500px]">
