        ...
        <div className="h-full flex items-center justify-center">
            ...
            >
                {[1, 2, 3].map((i) => (
                    <motion.div
                        key={i}
                        className="bg-muted-foreground/20 rounded-md h-5 w-full"
                        layout
                        transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }}
                    />
                ))}
            ...
            <div className="h-10 flex items-center justify-center overflow-hidden relative w-full">
                <AnimatePresence mode="wait">
                    {loading ? (
                        <motion.div
                            key="loader"
                            className="h-8 w-24 bg-muted/20 rounded"
                            initial={{ opacity: 0.5 }}
    ...
    return (
        <div className="flex items-center justify-center h-full gap-2">
            {shields.map((shield) => (
                <motion.div
                    key={shield.id}
                    className={`w-12 h-12 rounded-lg flex items-center justify-center ${shield.active ? 'bg-primary/20' : 'bg-muted/10'
                        }`}
                    animate={{ scale: shield.active ? 1.1 : 1 }}
                    transition={{ duration: 0.3 }}
                >
                    <Lock className={`w-5 h-5 ${shield.active ? 'text-primary' : 'text-muted-foreground'}`} />
                </motion.div>
            ))}
        </div>
    )
        ...
        <section className="bg-background px-6 py-24 min-h-screen flex items-center justify-center">
            <div className="max-w-7xl w-full mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-6 gap-4 auto-rows-[200px]">
                    ...
                    <motion.div
                        className="md:col-span-2 md:row-span-2 bg-card border border-border rounded-xl p-8 flex flex-col hover:border-primary/20 transition-colors cursor-pointer overflow-hidden"
                    ...
                    <motion.div
                        className="md:col-span-2 bg-card border border-border rounded-xl p-8 flex flex-col hover:border-primary/20 transition-colors cursor-pointer overflow-hidden"
                        initial={{ opacity: 0, y: 30 }}
                        whileInView={{ opacity: 1, y: 0 }}
                        viewport={{ once: true }}
                    ...
                    <motion.div
                        className="md:col-span-2 md:row-span-2 bg-card border border-border rounded-xl p-6 flex flex-col hover:border-primary/20 transition-colors cursor-pointer overflow-hidden"
                        initial={{ opacity: 0, y: 30 }}
                        whileInView={{ opacity: 1, y: 0 }}
                        viewport={{ once: true }}
                        transition={{ delay: 0.2 }}
                        whileHover={{ scale: 1.02, boxShadow: "0 25px 50px -12px rgba(var(--shadow-color), 0.5)" }}
                    >
                    ...
                    {/* 5. Security - Wide (3x1) */}
                    <motion.div
                        className="md:col-span-3 bg-card border border-border rounded-xl p-8 flex flex-col hover:border-primary/20 transition-colors cursor-pointer overflow-hidden"
                        initial={{ opacity: 0, y: 30 }}
                        whileInView={{ opacity: 1, y: 0 }}
                        viewport={{ once: true }}
